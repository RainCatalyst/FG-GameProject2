...
namespace SpaceGame
{
    public class CharacterMovement : MonoBehaviour
    {
        ...
        private Vector2 GetWallSteerDirection(Vector3 current)
        {
            ...
            if (Physics.Raycast(transform.position + Vector3.up * 0.5f, transform.forward, out var hit, 0.75f, _wallMask))
            {
                Vector3 right = Quaternion.AngleAxis(90, Vector3.up) * hit.normal;
                bool wallRight = Physics.Raycast(hit.point + right * _wallSteerOffset + hit.normal * 0.1f, -hit.normal, 0.5f,
                    _wallMask);
                bool wallLeft = Physics.Raycast(hit.point - right * _wallSteerOffset + hit.normal * 0.1f, -hit.normal, 0.5f,
                    _wallMask);
        ...
        [SerializeField]
        private float _wallSteerOffset = 0.5f;
